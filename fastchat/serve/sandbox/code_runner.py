...
DEFAULT_DATA_SCIENCE_PROMPT = """# Data Science Agent Protocol
...
If additional python packages are needed, output them in the markdown format:
***REMOTE SANDBOX PACKAGES***:
```
pip install package1 package2 ...
```
...
DEFAULT_JUPYTER_SANDBOX_INSTRUCTION = DEFAULT_DATA_SCIENCE_PROMPT
...
# + 'Sandbox Environment Name: ' + SandboxEnvironment.JUPYTER + '\n' + DEFAULT_DATA_SCIENCE_PROMPT.strip() + '\n------\n'
)
...
DEFAULT_SANDBOX_INSTRUCTIONS: dict[SandboxEnvironment, str] = {
    ...
    SandboxEnvironment.JUPYTER: DEFAULT_DATA_SCIENCE_PROMPT.strip(),
}
...
def on_run_code(
    ...
    match sandbox_env:
        ...
        case SandboxEnvironment.JUPYTER:
            output = run_code_interpreter(
                code=code, code_language='python', code_dependencies=code_dependencies
            )
            yield (
